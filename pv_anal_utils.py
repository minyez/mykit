...
class vasp_read_poscar():
    ...
    def __comment_info(poslines):
        poslines = self.poslines
        self.comment = poslines[0]

    def __atom_info(self):
        poslines = self.poslines
        self.atom_type = poslines[5].split()
        self.atom_num = [int(i) for i in poslines[6].split()]
    ...
    def __latt_info(self):
        poslines = self.poslines
        scale = float(poslines[1].split()[0])
        self.latt = []
        for i in xrange(3):
            self.latt.append([float(x)*scale for x in poslines[i+2].split()])
            ...
            for kp in xrange(self.nkp):
                for band in xrange(self.nbands):
                    for atom in xrange(self.natom):
                        ...
                        self.pwdata[spin,kp,band,atom] +=  data
