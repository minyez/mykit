...
def step_2_exact(chg_dir, exact_dir, tag_xc, encut, nks, nks_gw, nbands, nedos, lwannier,\
    ...
    with open('INCAR', 'w') as incar:
        vasp_write_incar_exact(incar, tag_xc, encut, nb=nbands, \
                npar=npar, mode_smear=smear, lwannier=lwannier)
    vasp_io_change_tag('INCAR', 'NEDOS', new_val=nedos)
    if f_metal:
        vasp_io_change_tag('INCAR', 'SIGMA', new_val=0.2)

# if kpoints for the first and second calculation is the same, 
# then it is not necessary to fix charge density, 
# and the wavefunction in Step 1 can be used
    if not nks == nks_gw:
...
# need to add more rules
    ...
    with open('INCAR', 'w') as incar:
        vasp_write_incar_exact(incar, tag_xc, encut, nb=nbands, npar=1, \
                mode_smear=smear, lwannier=lwannier)
        incar.write(' NEDOS = %s\n' % nedos)
...
# =================== Parser ==========================
    ...
    parser.add_argument("-e", dest='encut', type=float, default=0, \
            help="Planewave cutoff. 0 will set largest ENMAX")
    parser.add_argument("--egw", dest='encutgw', type=float, default=0, \
            help="dielectric cutoff for GW calculation, i.e. ENCUTGW")
    ...
    parser.add_argument("-k", dest='nk', default=[6, 6, 6], \
            help="Number of kpoints for SCF, format like [kx,ky,kz]")
    parser.add_argument("--kg", dest='nk_gw', default=[6, 6, 6], \
            help="Number of kpoints for GW, format like [kgx,kgy,kgz]")
    parser.add_argument("-m", dest='f_metal', action='store_true', \
            help="flag for metal system")
    parser.add_argument("-w", dest='nomega', default=50, \
    ...
    parser.add_argument("--gw", dest='gw_mode', default="G0W0", \
            help="Self-consistent level of GW. Default G0W0")
    parser.add_argument("--restart", dest='chg_start', action='store_true', \
            help="flag for restarting from SCF")
    parser.add_argument("--wannier", dest='lwannier', action='store_true', \
            help="flag for wannier90 to plot band structure")
    parser.add_argument("--test", dest='testmode', default=None, \
            help="flag for convergence test with respect to testmode, e.g. ENCUTGW, NBANDS")
...
# 3) run a GW calculation with WAVECAR and WAVEDER from Step 2
        ...
        step_2_exact(chg_dir, exact_dir, opts.tag_xc, opts.encut, nks, nks_gw, nbands, nedos, \
                lwannier=opts.lwannier, npar=npar, lmm=lmm, \
                vasp_cmd=vasp_cmd, f_metal=opts.f_metal, smear=[0, 0.05])

        print("== Step 3: GW                     ==")
        step_3_gw(exact_dir, gw_dir, opts.tag_xc, opts.encut, opts.encutgw, nbands=nbands, \
                nedos=nedos, nomega=opts.nomega, lwannier=opts.lwannier, \
                vasp_cmd=vasp_cmd, gw_mode=opts.gw_mode, f_metal=opts.f_metal, smear=[0, 0.05])
...
# IF testmode is set to ENCUTGW
        ...
        for x in xrange(3, 10):
            nbands_ori = nbands_scf + int((x + 1) * (mnpw - nbands_scf) / 10.0)
            nbands_ori = nbands_ori - nbands_ori % opts.nproc
            if nbands_ori not in nbands_list and (nbands_ori < mnpw):
                nbands_list.append(nbands_ori)

        for nb in nbands_list:
            print("== Step 2: Non-SCF for unc. bands ==")
            step_2_exact('../' + chg_dir, exact_dir, opts.tag_xc, opts.encut, \
                    nks, nks_gw, nbands=nb, nedos=nedos,\
                    lwannier=opts.lwannier, npar=npar, lmm=lmm, \
                    vasp_cmd=vasp_cmd, f_metal=opts.f_metal, smear=[0, 0.05])
            print("== Step 3:    GW   BANDS:%5s    ==" % nb)
            step_3_gw(exact_dir, gw_dir + '_nb_' + str(nb), opts.tag_xc, opts.encut, opts.encutgw, \
                  nbands=nb, nedos=nedos, nomega=opts.nomega, lwannier=opts.lwannier, \
                  vasp_cmd=vasp_cmd, gw_mode=opts.gw_mode, f_metal=opts.f_metal, smear=[0, 0.05])
        ...
        with open('Gap_NBANDS.dat', 'w') as f:
            f.write("#MNPW=%6s\n" % mnpw)
            f.write("#NBANDS gap\n")
            for i in range(len(nbands_list)):
...
# IF testmode is set to ENCUTGW
        ...
        print("== Step 2: Non-SCF for unc. bands ==")
        step_2_exact('../'+chg_dir, exact_dir, opts.tag_xc, opts.encut, \
                nks, nks_gw, nbands=nbands, nedos=nedos, lwannier=opts.lwannier,\
                npar=npar, lmm=lmm, vasp_cmd=vasp_cmd, f_metal=opts.f_metal, smear=[0, 0.05])
        egw_list = [encut / 9.0*x for x in range(2, 9, 2)] # up to half of the ENCUT
        for egw in egw_list:
            print("== Step 3:    GW ENCUTGW:%7.1f  ==" % egw)
            step_3_gw(exact_dir, gw_dir+'_egw_'+str(int(egw)), opts.tag_xc, opts.encut, egw, \
                  nbands=nbands, nedos=nedos, encutgw=egw, nomega=opts.nomega, \
                  lwannier=opts.lwannier, \
                  vasp_cmd=vasp_cmd, gw_mode=opts.gw_mode, f_metal=opts.f_metal, smear=[0, 0.05])
